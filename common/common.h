...
struct common_params {
    ...
    // HyphalLLM: PhysarumFormer + HyphalMemory options
    bool        use_hyphal        = false;   // enable bio-inspired memory
    int32_t     hyphal_nodes      = 512;     // max active nodes in HyphalGraph
    float       hyphal_ssm_frac   = 0.65f;  // fraction of layers in SSM zone
    float       hyphal_gate_frac  = 0.10f;  // fraction of layers in gate zone
    std::string hyphal_save_path  = "";      // save graph after session
    std::string hyphal_load_path  = "";      // load graph at startup
