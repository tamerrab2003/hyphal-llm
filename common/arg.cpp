...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    // HyphalLLM: PhysarumFormer arguments
    add_opt(common_arg(
        {"--hyphal-nodes"}, "N",
        "HyphalLLM: max active nodes in HyphalGraph (default: 512)\n"
        "Higher = better quality, more RAM. 512=25MB, 4096=200MB, 16384=800MB.",
        [](common_params & params, int value) {
            params.hyphal_nodes = value;
            params.use_hyphal   = true;
        }
    ).set_env("LLAMA_ARG_HYPHAL_NODES"));
    add_opt(common_arg(
        {"--hyphal-ssm-frac"}, "F",
        "HyphalLLM: fraction of layers in SSM zone O(n) (default: 0.65)\n"
        "0.65 = 65%% of layers use linear SSM, 25%% full attention.",
        [](common_params & params, const std::string & value) {
            params.hyphal_ssm_frac = std::stof(value);
            params.use_hyphal      = true;
        }
    ).set_env("LLAMA_ARG_HYPHAL_SSM_FRAC"));
    add_opt(common_arg(
        {"--hyphal-gate-frac"}, "F",
        "HyphalLLM: fraction of layers in adaptive gate zone (default: 0.10)",
        [](common_params & params, const std::string & value) {
            params.hyphal_gate_frac = std::stof(value);
            params.use_hyphal       = true;
        }
    ).set_env("LLAMA_ARG_HYPHAL_GATE_FRAC"));
    add_opt(common_arg(
        {"--hyphal-save"}, "PATH",
        "HyphalLLM: save Physarum graph after session (enables cross-session learning)",
        [](common_params & params, const std::string & value) {
            params.hyphal_save_path = value;
            params.use_hyphal       = true;
        }
    ).set_env("LLAMA_ARG_HYPHAL_SAVE"));
    add_opt(common_arg(
        {"--hyphal-load"}, "PATH",
        "HyphalLLM: load previously saved Physarum graph at startup",
        [](common_params & params, const std::string & value) {
            params.hyphal_load_path = value;
            params.use_hyphal       = true;
        }
    ).set_env("LLAMA_ARG_HYPHAL_LOAD"));
    add_opt(common_arg(
        {"--hellaswag"},
