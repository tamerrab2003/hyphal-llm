...
llama_context::llama_context(
    ...
    if (!hparams.vocab_only) {
#ifdef HYPHAL_ENABLED
        // HyphalLLM: if --hyphal-nodes > 0, use HyphalMemory instead of KV cache
        // This is the real replacement — standard KV cache is never allocated.
        if (params.n_hyphal_nodes > 0) {
            auto * hmem = new llama_hyphal_memory(
                hparams, cparams,
                (uint32_t)params.n_hyphal_nodes,
                params.hyphal_load_path);
            memory.reset(hmem);
            LLAMA_LOG_INFO("%s: HyphalMemory: %d nodes × %d layers = %.1f MB"
                           " (vs %.1f MB standard KV at ctx=%d)\n",
                __func__,
                params.n_hyphal_nodes,
                (int)hparams.n_layer,
                hmem->graph().memory_bytes() / 1e6f,
                (float)cparams.n_ctx * hparams.n_layer *
                hparams.n_head_kv() * hparams.n_embd_head_k() * 2 * 2 / 1e6f,
                (int)cparams.n_ctx);
        } else {
#endif
...
llama_context_params llama_context_default_params() {
    llama_context_params result = {
        ...
        /*.n_hyphal_nodes              =*/ 0,       // 0 = disabled, use standard KV cache
        /*.hyphal_load_path            =*/ nullptr,
        /*.hyphal_save_path            =*/ nullptr,
...
#ifdef HYPHAL_ENABLED
        // HyphalLLM: initialise Physarum state when context is created
        // Uses default zone fractions (65% SSM, 10% gate, 25% full).
        // Override with --hyphal-ssm-frac / --hyphal-gate-frac CLI flags.
        {
            const int n_layer = (int)model->hparams.n_layer;
            const int n_head  = (int)model->hparams.n_head();
            physarum_init(n_layer, n_head, 0.65f, 0.10f);
            LLAMA_LOG_INFO("%s: HyphalLLM physarum_init: %d layers %d heads\n",
                           __func__, n_layer, n_head);
        }
#endif
        return ctx;
    } catch (const std::exception & err) {
...
void llama_free(llama_context * ctx) {
    ...
    if (hyphal_session_is_active()) {
        LLAMA_LOG_INFO("%s: HyphalLLM step=%d active=%d/%d sparsity=%.1f%%\n",
            __func__, g_physarum_step,
            physarum_active_heads_total(),
            g_physarum_n_layers * g_physarum_n_heads,
            physarum_global_sparsity() * 100.0f);
