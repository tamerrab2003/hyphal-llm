...
static void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor) {
    ...
    switch (tensor->op) {
        ...
        case GGML_OP_HYPHAL_ATTEND:
            /* HyphalLLM: handled by physarum_state.c at the llama layer level.
             * The GGML op is a no-op here — conductance weighting is applied
             * directly in the llama model's layer loop via physarum_get_zone(). */
            break;
        case GGML_OP_PHYSARUM_ROUTE:
            /* HyphalLLM: routing decision — no tensor computation needed. */
            break;
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        if (!ggml_cpu_extra_work_size(n_threads, node, &cur)) {
            switch (node->op) {
                ...
                case GGML_OP_HYPHAL_ATTEND:
                case GGML_OP_PHYSARUM_ROUTE:
                    /* HyphalLLM: no scratch memory needed — ops handled at llama layer */
                    break;
