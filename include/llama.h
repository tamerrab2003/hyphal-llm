...
extern "C" {
    ...
    struct llama_context_params {
        ...
        // HyphalLLM: when > 0, use HyphalMemory instead of standard KV cache.
        // Memory is fixed at: n_hyphal_nodes * n_layer * n_embd_kv * 4 bytes.
        // Example: 512 nodes, Qwen3-7B (32L, 8KVH, 128D) = ~128 MB vs 4 GB at 32K ctx.
        int32_t     n_hyphal_nodes;   // 0 = disabled (default), 512/1024/4096 = enabled
        const char* hyphal_load_path; // path to load saved graph (NULL = fresh start)
        const char* hyphal_save_path; // path to save graph on exit (NULL = no save)
